...
In this technical report, we propose \textbf{Stabilized Piecewise-Rational Charts (SPRC)}, a constructive algebraic method that parameterizes the Birkhoff polytope for $n=4$ without iteration. By employing a smooth tropical norm (LogSumExp) and progressive saturation ($\tanh$), our method ensures constraint satisfaction by construction. Benchmarks on NVIDIA T4 GPUs demonstrate a \textbf{13.3$\times$ speedup} in kernel execution and a \textbf{2.53$\times$ reduction} in end-to-end training time compared to the Sinkhorn baseline ($t_{max}=20$), while achieving equivalent convergence properties.
\end{abstract}

\section{Introduction}
Recent work by DeepSeek-AI \cite{deepseek2024} introduced Manifold-Constrained Hyper-Connections (mHC) to restore the identity mapping property in expanded residual streams. While effective, the reliance on the Sinkhorn-Knopp algorithm ($t_{max}=20$) for manifold projection imposes a "memory wall" bottleneck. We present an algebraic parametrization that eliminates these loops, minimizing latency overhead while maintaining the representational capacity of the layer.
...
Recall the tight bound property of the LogSumExp function for $N$ elements with scaling factor $\eta$:
\begin{equation}
    \max_{k}(x_k) \le \eta \log \left( \sum_{k=1}^{N} \exp(x_k/\eta) \right) \le \max_{k}(x_k) + \eta \log(N)
\end{equation}
In our case, $N=16$ and the inputs are $-V_{ij}$. Thus, we have the lower bound $m_{smooth}(V) \ge \max_{i,j}(-V_{ij})$. 
...
\end{equation}
Since $-V_{ij} \le m_{smooth}(V)$, the fraction is strictly less than 1 (given $\epsilon > 0$). Thus $H_{ij} > 0$ is strictly guaranteed.
...
The kernel speedup is $13.3\times$, while the end-to-end training speedup is $2.53\times$. This discrepancy is consistent with Amdahl's Law. Our profiling suggests that in the baseline implementation, the iterative Sinkhorn operations accounted for approximately \textbf{60\%} of the total layer execution time. By reducing this cost to near-zero ($O(1)$) with SPRC, the theoretical maximum speedup is $\approx 1 / (1 - 0.6) = 2.5\times$, which aligns closely with our empirical result of $2.53\times$.
...
\begin{figure}[h]
\centering
% Ensure 'loss_plot.png' is in the same directory
\includegraphics[width=0.8\textwidth]{loss_plot3.png}
\caption{Training Loss Convergence\protect\footnotemark. Our method (Orange) tracks the Sinkhorn baseline (Blue) closely, demonstrating that the algebraic parametrization preserves the layer's expressivity.}
\label{fig:loss}
\end{figure}

\footnotetext{Experimental settings: Batch size 64, Sequence length 16, Dim 256, Learning rate $1e^{-3}$, Optimizer AdamW, 500 steps, FP32 precision.}

\section{Conclusion}
We proposed a smooth algebraic parametrization for mHC layers that replaces iterative approximation with an exact, differentiable closed-form solution. The method achieves a 13.3$\times$ kernel speedup and practically identical convergence behavior, offering a robust and efficient alternative for large-scale model training.
